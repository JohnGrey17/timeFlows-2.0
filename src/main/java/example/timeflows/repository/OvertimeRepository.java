...
public interface OvertimeRepository extends JpaRepository<Overtime, Long> {
    ...
    @EntityGraph(
            attributePaths = {"user", "user.roles", "user.division", "user.division.department"})
    List<Overtime> findByUserEmailAndWorkDateBetweenOrderByWorkDateAsc(
            String email, LocalDate from, LocalDate to);

    @EntityGraph(
            attributePaths = {"user", "user.roles", "user.division", "user.division.department"})
    List<Overtime> findByUserDivisionIdAndWorkDateBetweenOrderByUserEmailAscWorkDateAsc(
            Long divisionId, LocalDate from, LocalDate to);

    @EntityGraph(
            attributePaths = {"user", "user.roles", "user.division", "user.division.department"})
    List<Overtime> findByUserDivisionDepartmentIdAndWorkDateBetweenOrderByUserEmailAscWorkDateAsc(
            Long departmentId, LocalDate from, LocalDate to);

    @EntityGraph(
            attributePaths = {"user", "user.roles", "user.division", "user.division.department"})
    List<Overtime> findByUserIdAndWorkDateBetweenOrderByWorkDateAsc(
